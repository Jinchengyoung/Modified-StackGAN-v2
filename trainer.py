...
class GANTrainer(object):
    ...
    def train(self):
        ...
        #JC
        # nl = cfg.GAN.P_DIM
        # phy = torch.FloatTensor(self.batch_size,nl)
        # physics = Variable(phy.fill_(torch.randn(1)))
        nl = cfg.GAN.P_DIM
        phy = torch.FloatTensor(self.batch_size,nl)
          # randomNo = (1 - (-1)) * torch.rand(1) -1
          # randomNo = randomNo.item()
          # physics = Variable(phy.new_full((self.batch_size,nl), randomNo))
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            for step, data in enumerate(self.data_loader, 0):
                ...
                physics = Variable(phy.new_full((self.batch_size,nl), randomNo))
                physics.cuda()
                self.fake_imgs, _, _ = self.netG(noise, physics) #JC
                #######################################################
                # (2) Update D network
                ######################################################
                errD_total = 0
                ...
                if count % cfg.TRAIN.SNAPSHOT_INTERVAL == 0:
                    ...
                    self.fake_imgs, _, _ = self.netG(fixed_noise, physics)#JC
    ...
    def evaluate(self, split_dir):
        if cfg.TRAIN.NET_G == '':
            ...
            noise = Variable(torch.FloatTensor(self.batch_size, nz))
            #JC###################################
            randomNo1 = (1 - (0)) * torch.rand(1) -1
            randomNo1 = randomNo1.item()
            noise1 = torch.FloatTensor(self.batch_size,nz)
            #noise1 = Variable(noise1.new_full((self.batch_size,nz), randomNo1))
            noise1 = Variable(noise1.new_full((self.batch_size,nz), 0.1))
            nl = cfg.GAN.P_DIM
            phy = torch.FloatTensor(self.batch_size,nl)
            randomNo = (1 - (-1)) * torch.rand(1) -1
            randomNo = randomNo.item()
            physics = Variable(phy.new_full((self.batch_size,nl), randomNo)) 
            #physics1 = Variable(torch.FloatTensor(self.batch_size,nl))
            #physics = Variable(phy.new_full((self.batch_size,nl), 0.2)) # JC
            ########################################
            if cfg.CUDA:
                netG.cuda()
                noise = noise.cuda()
            ...
            for step in range(num_batches):
                noise.data.normal_(0, 1)
                #physics.data.normal_(0.1, 0.5)
                fake_imgs, _, _ = netG(noise, physics)#JC
                #fake_imgs, _, _ = netG(noise1, physics)#JC
                #fake_imgs, _, _ = netG(noise1, physics1)#JC
                if cfg.TEST.B_EXAMPLE:
