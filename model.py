...
class INIT_STAGE_G(nn.Module):
    def __init__(self, ngf):
        ...
        if cfg.GAN.B_CONDITION:
            ...
            self.in_dim = cfg.GAN.Z_DIM + cfg.GAN.P_DIM #JC
        self.define_module()
    ...
    def forward(self, z_code, c_code=None):
        ...
        #else:
            #in_code = z_code
        elif c_code is not None:#JC
            in_code = torch.cat((c_code, z_code), 1)#JC
...
class G_NET(nn.Module):
    ...
    #def forward(self, z_code, p_code, text_embedding=None):
    def forward(self, z_code, p_code):#JC
        #if cfg.GAN.B_CONDITION and text_embedding is not None:
            #c_code, mu, logvar = self.ca_net(text_embedding)
        if cfg.GAN.B_CONDITION and p_code is not None:#JC
            c_code, mu, logvar = self.ca_net(p_code)#JC
        elif p_code is not None:
            #c_code, mu, logvar = z_code, None, None
            c_code, mu, logvar = p_code, None, None
        fake_imgs = []
        if cfg.TREE.BRANCH_NUM > 0:
